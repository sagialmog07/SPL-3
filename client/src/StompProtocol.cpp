#include "StompProtocol.h"
#include "event.h"
#include <iostream>
#include <sstream>

StompProtocol::StompProtocol(ConnectionHandler& handler) 
    : connectionHandler(handler), dataManager(), terminate(false), 
      loggedIn(false), currentUser(""), nextSubscriptionId(0), receiptCounter(1000),
      dataMapsMutex(), topicSubscriptions(), activeReceipts() {}

std::string StompFrame::toString() const {
    std::string result = command + "\n";
    for (auto const& pair : headers) {
        result += pair.first + ":" + pair.second + "\n";
    }
    result += "\n"; // השורה הריקה המפרידה בין ה-Headers ל-Body
    result += body;
    // שים לב: אנחנו לא מוסיפים כאן \0 כי ה-ConnectionHandler מוסיף אותו בשליחה
    return result; 
}

bool StompProtocol::shouldTerminate() const { return terminate; }
int StompProtocol::generateReceiptId() { return receiptCounter++; }
int StompProtocol::getNextSubscriptionId() { return nextSubscriptionId++; }

std::string StompProtocol::processCommand(const std::string& command) {
    if (command.empty()) return "";
    std::istringstream iss(command);
    std::string cmdType;
    iss >> cmdType;

    if (cmdType == "login") return handleLogin(command);
    if (!loggedIn) { std::cout << "Error: Please login first.\n"; return ""; }
    if (cmdType == "join") return handleJoin(command);
    if (cmdType == "exit") return handleExit(command);
    if (cmdType == "report") return handleReport(command);
    if (cmdType == "summary") return handleSummary(command);
    if (cmdType == "logout") return handleLogout(command);
    return "";
}

std::string StompProtocol::handleLogin(const std::string& command) {
    std::istringstream iss(command);
    std::string cmd, host_port, user, pass;
    iss >> cmd >> host_port >> user >> pass;
    
    // התיקון: הורדנו מכאן את ה-connectionHandler.connect כי זה כבר קורה ב-main!
    // אנחנו רק שומרים את הנתונים ובונים את הפריים.

    loggedIn = true; 
    currentUser = user;
    
    std::map<std::string, std::string> h;
    h["accept-version"] = "1.2"; 
    h["host"] = "stomp.cs.bgu.ac.il";
    h["login"] = user; 
    h["passcode"] = pass;
    
    return buildFrame("CONNECT", h, "");
}

std::string StompProtocol::handleReport(const std::string& command) {
    std::istringstream iss(command);
    std::string cmd, path;
    iss >> cmd >> path;

    try {
        // קריאת האירועים מהקובץ
        names_and_events ne = parseEventsFile(path);
        std::string channel = ne.team_a_name + "_" + ne.team_b_name;

        std::cout << ">>> Reporting " << ne.events.size() << " events to channel: " << channel << std::endl;

        for (const auto& e : ne.events) {
            // בניית ההדרים (Headers)
            std::map<std::string, std::string> h;
            h["destination"] = "/" + channel;

            // בניית גוף ההודעה (Body) בפורמט הנדרש
            std::string b = "user: " + currentUser + "\n";
            b += "team a: " + ne.team_a_name + "\n";
            b += "team b: " + ne.team_b_name + "\n";
            b += "event name: " + e.get_name() + "\n";
            b += "time: " + std::to_string(e.get_time()) + "\n";
            b += "general game updates:\n";
            for (auto const& u : e.get_game_updates()) b += "\t" + u.first + ": " + u.second + "\n";
            b += "team a updates:\n";
            for (auto const& u : e.get_team_a_updates()) b += "\t" + u.first + ": " + u.second + "\n";
            b += "team b updates:\n";
            for (auto const& u : e.get_team_b_updates()) b += "\t" + u.first + ": " + u.second + "\n";
            b += "description:\n" + e.get_discription() + "\n";

            // יצירת ה-Frame ושליחה לסוקט
            std::string frameStr = buildFrame("SEND", h, b);
            if (!connectionHandler.sendFrameAscii(frameStr, '\0')) {
                std::cout << "Fatal Error: Connection lost while sending reports." << std::endl;
                break;
            }
        }
        std::cout << ">>> Report sending finished." << std::endl;

    } catch (const std::exception& ex) {
        std::cout << "Error: Could not process report file: " << ex.what() << std::endl;
    } catch (...) {
        std::cout << "Error: Unknown error in handleReport." << std::endl;
    }

    return ""; 
}

void StompProtocol::handleMessage(const StompFrame& frame) {
    try {
        std::cout << "\n----- New Event Received -----\n" << frame.body << std::endl;

        std::string dest = frame.headers.at("destination");
        std::string channel = (dest[0] == '/') ? dest.substr(1) : dest;
        std::istringstream ss(frame.body);
        std::string line, user, tA, tB, eName, desc, section;
        int time = 0; std::map<std::string, std::string> gen, updA, updB;

        while(std::getline(ss, line)) {
            if (line.empty() || line == "\r") continue;
            if (line.back() == '\r') line.pop_back();

            // בדיקות בטיחות לפני substr
            if (line.find("user: ") == 0 && line.length() > 6) user = line.substr(6);
            else if (line.find("team a: ") == 0 && line.length() > 8) tA = line.substr(8);
            else if (line.find("team b: ") == 0 && line.length() > 8) tB = line.substr(8);
            else if (line.find("event name: ") == 0 && line.length() > 12) eName = line.substr(12);
            else if (line.find("time: ") == 0 && line.length() > 6) {
                try { time = std::stoi(line.substr(6)); } catch(...) { time = 0; }
            }
            else if (line == "general game updates:") section = "g";
            else if (line == "team a updates:") section = "a";
            else if (line == "team b updates:") section = "b";
            else if (line == "description:") section = "d";
            else if (!line.empty() && line[0] == '\t') {
                size_t c = line.find(':');
                if (c != std::string::npos && line.length() > c + 2) {
                    std::string k = line.substr(1, c-1), v = line.substr(c+2);
                    if (section == "g") gen[k] = v; 
                    else if (section == "a") updA[k] = v; 
                    else if (section == "b") updB[k] = v;
                }
            } else if (section == "d") desc += line + "\n";
        }
        Event ev(tA, tB, eName, time, gen, updA, updB, desc);
        dataManager.addEvent(channel, user, ev);
        
    } catch (const std::exception& e) {
        std::cout << "DEBUG: Error parsing MESSAGE frame: " << e.what() << std::endl;
    }
}

void StompProtocol::handleReceipt(const StompFrame& frame) {
    if (frame.headers.count("receipt-id")) {
        int rId = std::stoi(frame.headers.at("receipt-id"));
        std::lock_guard<std::mutex> lock(dataMapsMutex);
        if (activeReceipts.count(rId)) {
            StompReceipt receipt = activeReceipts[rId];
            if (receipt.actionType == "JOIN") std::cout << "Joined channel " << receipt.destinationChannel << std::endl;
            else if (receipt.actionType == "EXIT") { std::cout << "Exited channel " << receipt.destinationChannel << std::endl; topicSubscriptions.erase(receipt.destinationChannel); }
            else if (receipt.actionType == "LOGOUT") { std::cout << "Logout successful\n"; terminate = true; connectionHandler.close(); }
            activeReceipts.erase(rId);
        }
    }
}

StompFrame StompProtocol::parseRawFrame(const std::string& raw) {
    StompFrame f; std::istringstream ss(raw); std::string line;
    if (std::getline(ss, line)) {
        while (!line.empty() && (line.back() == '\r' || line.back() == '\n')) line.pop_back();
        f.command = line;
    }
    while (std::getline(ss, line) && line != "" && line != "\r") {
        if (!line.empty() && line.back() == '\r') line.pop_back();
        size_t c = line.find(':');
        if (c != std::string::npos) {
            std::string key = line.substr(0, c), val = line.substr(c+1);
            if (!val.empty() && val[0] == ' ') val.erase(0, 1); // ניקוי רווח
            f.headers[key] = val;
        }
    }
    while (std::getline(ss, line)) f.body += line + "\n";
    return f;
}

std::string StompProtocol::handleJoin(const std::string &command) {
    std::istringstream iss(command); std::string cmd, channel; iss >> cmd >> channel;
    int rId = generateReceiptId(); int sId = getNextSubscriptionId();
    { std::lock_guard<std::mutex> lock(dataMapsMutex); activeReceipts[rId] = StompReceipt("JOIN", channel); topicSubscriptions[channel] = sId; }
    std::map<std::string, std::string> h; h["destination"] = "/" + channel; h["id"] = std::to_string(sId); h["receipt"] = std::to_string(rId);
    return buildFrame("SUBSCRIBE", h, "");
}

std::string StompProtocol::handleExit(const std::string& command) {
    std::istringstream iss(command); 
    std::string cmd, channel; 
    iss >> cmd >> channel;

    int rId = generateReceiptId(); 
    int sId = -1;

    { 
        std::lock_guard<std::mutex> lock(dataMapsMutex); 
        if(topicSubscriptions.count(channel)) {
            sId = topicSubscriptions[channel];
            activeReceipts[rId] = StompReceipt("EXIT", channel); 
        }
    }

    if (sId == -1) {
        std::cout << "Error: You are not subscribed to channel " << channel << std::endl;
        return "";
    }

    std::map<std::string, std::string> h; 
    h["id"] = std::to_string(sId); 
    h["receipt"] = std::to_string(rId);
    
    return buildFrame("UNSUBSCRIBE", h, "");
}

std::string StompProtocol::handleSummary(const std::string& command) {
    std::istringstream iss(command); std::string cmd, game, user, file; iss >> cmd >> game >> user >> file;
    dataManager.generateSummary(game, user, file); return "";
}

std::string StompProtocol::handleLogout(const std::string& command) {
    int rId = generateReceiptId();
    { std::lock_guard<std::mutex> lock(dataMapsMutex); activeReceipts[rId] = StompReceipt("LOGOUT", ""); }
    std::map<std::string, std::string> h; h["receipt"] = std::to_string(rId);
    return buildFrame("DISCONNECT", h, "");
}

void StompProtocol::processFrame(const StompFrame& f) {
    if (f.command == "CONNECTED") std::cout << "Login successful" << std::endl;
    else if (f.command == "RECEIPT") handleReceipt(f);
    else if (f.command == "MESSAGE") handleMessage(f);
    else if (f.command == "ERROR") handleError(f);
}

void StompProtocol::handleError(const StompFrame& frame) {
    std::string msg = frame.headers.count("message") ? frame.headers.at("message") : "Unknown Error";
    std::cout << "Server Error: " << msg << std::endl;
    terminate = true;
    connectionHandler.close();
}

std::string StompProtocol::buildFrame(const std::string& command, const std::map<std::string, std::string>& headers, const std::string& body) {
    StompFrame frame; frame.command = command; frame.headers = headers; frame.body = body; return frame.toString();
}